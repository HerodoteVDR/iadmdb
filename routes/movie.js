...
let randomTeacher = getRandomTeacher();
...
async function getTeacherImage() {
    ...
    return base('Teachers')
        ...
        .firstPage()
        .then((data) => {
            image = data.map((d) => d.get("Images"));
            image = image[randomTeacher];
            return image
        })
...
router.get('/id/*', async function (req, res, next) {
    ...
    else if (cast.cast[0].popularity > 5 && cast.cast[0].popularity < 5) {
...
router.get('/search/*', async function (req, res, next) {
    ...
    // Get movie data from tmdb
    const movies = await getSpecificMovie(requested_movie_n);
